...
def ppf(q: ArrayLike, mu: ArrayLike, loc: ArrayLike = 0) -> Array:
  r"""Poisson percent point function (inverse of cdf).

  JAX implementation of :obj:`scipy.stats.poisson` ``ppf``.

  The percent point function is the inverse of the cumulative distribution
  function.

  Args:
    q: arraylike, lower tail probability
    mu: arraylike, distribution shape parameter
    loc: arraylike, distribution offset parameter
  ...
  See Also:
    ...
    - :func:`jax.scipy.stats.poisson.cdf`
  """
  q, mu, loc = promote_args_inexact("poisson.ppf", q, mu, loc)
  zero = _lax_const(q, 0)
  one = _lax_const(q, 1)
  mu = jnp.maximum(mu, 0)
  # Initial bound for search
  k = jnp.maximum(0, jnp.floor(mu + 5.0 * jnp.sqrt(mu))).astype(jnp.int32)

  def body_fun(k, _):
    cdf_k = cdf(k, mu)
    k = jnp.where(cdf_k < q, k + 1, k)
    return k, None

  def cond_fun(val):
    k, _ = val
    cdf_k = cdf(k, mu)
    return jnp.any(cdf_k < q)

  k = lax.while_loop(cond_fun, body_fun, (k, None))[0]
  return jnp.where(jnp.logical_or(lax.lt(q, zero), lax.gt(q, one)), jnp.nan,
                   lax.add(k, loc))
